...
class M_data extends CI_Model
{
	...
	function sess_thakademik_capture($tbl1)
	{
		$this->db->select( 
		"bimbingan_capture.kd_capture, 
		 bimbingan_capture.kd_bimbingan AS Kode Bimbingan,
		 bimbingan_capture.thn_akademik AS Tahun Akademik"
	);
		$this->db->from($tbl1);
		$this->db->join(
			"bimbingan_data",
			"bimbingan_data.kd_bimbingan = bimbingan_capture.kd_bimbingan"
		);
		// $this->db->join();
		$query = $this->db->get();
		return $query;
	...
	function get_bimbingan($tbl1, $kd_bimbingan){
		$this->db->select( 
		   "bimbingan_data.kd_bimbingan AS kd_bimbingan,bimbingan_data.topik AS topik, 
			bimbingan_data.judul AS judul, bimbingan_data.nama_dosen AS dospem, bimbingan_data.thn_akademik AS thn_akademik"
		);
			$this->db->from($tbl1);
			$this->db->join(
				"user",
				"user.kd_bimbingan = bimbingan_data.kd_bimbingan"
			);
			$this->db->where('user.kd_bimbingan',$kd_bimbingan);
			// $this->db->join();
			$query = $this->db->get();
			return $query;
	...
	// Yang Sudah Fix
	function get_session($tbl1){
		$this->db->select(
			"bimbingan_informasi.kd_informasi AS kd_informasi, bimbingan_informasi.fakultas_info AS fakultas_info,
			bimbingan_informasi.prodi_info AS prodi_info, setting.thn_akademik AS thn_akademik"
		);
		$this->db->from($tbl1);
		$this->db->join(
			"setting",
			"setting.thn_akademik = bimbingan_informasi.thn_akademik"
		);
		// $this->db->join(
		// 	"bimbingan_capture",
		// 	"bimbingan_capture.thn_akademik = bimbingan_informasi.thn_akademik"
		// );
		$query = $this->db->get();
		return $query;
	}
...
// Docs
	...
	function CheckLoginModel($npm, $password)
	{
		$this->db->where('npm', $email);
